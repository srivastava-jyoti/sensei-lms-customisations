...
// use Sensei\Includes\Admin\Sensei_Learner_Management;
		...
		if ( 'auto' === $quiz_grade_type && ! is_wp_error( $grade ) ) {
			...
			if ( 'on' === $pass_required ) {
				...
				if ( $quiz_pass_percentage <= $grade ) {
					...
					if ($user_id) {
						$post_id = get_the_ID();
						if($post_id == 4039 || $post_id == 4413){
							$post_attempts_key = 'attempts_by_user_post_' . $post_id;
							$attempts = get_user_meta($user_id, $post_attempts_key, true);
							$attempts = empty($attempts) ? 0 : (int) $attempts;
							$attempts++;
							update_user_meta($user_id, 'attempts_by_user_post_' . $post_id, $attempts);
							if($attempts >1){
							$course_id = Sensei()->lesson->get_course_id( $lesson_id );
