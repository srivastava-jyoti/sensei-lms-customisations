...
class Sensei_Course_Theme_Lesson {
	...
	private function maybe_add_quiz_results_notice() {
		...
		// var_dump($quiz_progress->get_status()); die();
		...
		if ($quiz_id === 4039 && $quiz_progress->get_status() === 'failed'){
		// var_dump("hello"); die();
		}
	}
	...
	private function maybe_add_lesson_prerequisite_notice() {
		...
		if ( $lesson_prerequisite > 0 ) {
			...
			$text = ! empty( $prerequisite_progress ) && 'ungraded' === $prerequisite_progress->get_status()
				...
				// translators: Placeholder is the link to the prerequisite lesson.
				: sprintf( esc_html__( 'Due to 2 failed attempts to reach a 100%% score on this exam, your course progress has been reset. To fulfill the %1$s, please restart the course and complete each module before making another attempt at this final exam.', 'sensei-lms' ), $prerequisite_lesson_link );
